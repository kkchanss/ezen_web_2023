...
public class Ex1_실행 {
	...
	public static void main(String[] args) {
		...
		// System.out.println(phone.wifi); // 오류 발생 : 해당 객체는 부모 객체이고 해당 메소드는 자식 클래스의 멤버이므로 불가능
		...
		// phone.setWifi(true); // 오류 발생 : 해당 객체는 부모 객체이고 해당 메소드는 자식 클래스의 멤버이므로 불가능
		// 2. SmartPhone 자식 객체
		...
		myPhone.hangUp();
		myPhone.setWifi(true); // 오류 발생 없는 이유 : 해당 객체는 자식객체이고, 메소드는 자식클래스의 멤버이므로 사용 가능
		myPhone.internet();
	}
