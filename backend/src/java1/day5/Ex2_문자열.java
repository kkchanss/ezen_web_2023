...
public class Ex2_문자열 {
	...
	public static void main(String[] args) {
		...
		if(name3 == name4) { // " " 문자열리터럴 생성된 문자열객체 비교
...
/*
	...
	자바 문자열에서 제공해주는 라이브러리 = 미리 만들어진 함수들	
		객체.메소드명()
		1. 문자열객체1.equals(문자열객체)	: 두 문자열객체내 문자열이 동일하면 true 아니면 false 반환 [주소x]
		2. 문자열객체.charAt(인덱스)		: 문자열내 해당 인덱스의 문자1개 추출 [ 첫번째 글자 = 인덱스 0 ]
		3. 문자열객체.length()				: 문자열에서 문자의 개수 반환
			vs 배열명.length
		4. 문자열객체.replace("기존문자열", "새로운문자열"); : 문자열 내 기존문자열이 존재하면 새로운 문자열로 치환하기
		5. 
			문자열객체.substring(인덱스)				: 해당 인덱스부터 끝까지 문자열 추출 
			문자열객체.substring(인덱스, 인덱스)		: 시작 인덱스부터 마지막인덱스 전까지 문자열 추출
