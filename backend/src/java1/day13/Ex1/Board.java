...
public class Board {
	...
	// 1. 필드 : 객체의 데이터 저장하는 곳
		/*
		 	- 필드 종류
		 		- 인스턴스필드 			: (힙영역) 객체 생성시 객체 안에 저장되어 있는 필드 - 사용
		 		- 정적필드				: (스택영역) [static] 프로그램 실행시 우선할당 객체없이 저장되어 있는 필드 - 사용
			...
			- 기타 키워드
		 	...
		 	- 싱글톤				: 프로그램 내 1개만 존재하는 객체
		 		...
		 		2. public static getInstance() 싱글톤 객체 반환
	...
	// 2. 생성자 : 객체 초기화/[처음값대입] 할때 사용
		/*
			...
			- 생성자 종류 [ 오버로딩으로 인해 다양하게 선언 가능 ]
				- 오버로딩 : 매개변수의 개수, 순서, 타입에 따라서 식별 가능한 거
				
			- 접근 제한 private, public default
			
			- 기타 키워드
				- this				: (생략가능) 현재 클래스의 구성멤버 식별할때 사용
				- this.필드명			: 외부로부터 들어온 매개변수명과 현재 클래스 내 필드명과 같을때 필드를 식별할 때
				- this.()			: 현재 클래스의 생성자 호출
				- this.메소드명()		: 현재 클래스의 메소드 호출
		 */
	// 3. 메소드 : 객체으 행동/동작 수행할때 사용
	/*
	 	...
	 	- 메소드 선언
	 		접근제한자 리턴타입
			 	...
			 	- return : ( void 일때 생략 가능) 리턴타입에 맞는 값을 리턴후 메소드 종료
	...
	/*
		...
		- MVC 디자인 패턴 : 기능역할에 따른 구분
			...
			M : 데이터 모델과 실제 데이터
				DTO : 데이터 이동 객체 [ 서로 다른 레이어간 이동할 때 객체 사용 ]
					1. 이동 목적
						...
						- DTO
					2. DTO 가독성
						member : 회원객체인데 회원 무슨 객체?
						memberDTO : 회원객체인데.. 이동객체이구나 파악할 수 있다.
				DAO : 데이터 접근 객체 [ 실제 원본 데이터(DB) 객체만 ]
