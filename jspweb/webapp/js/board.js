			...
			let board_list =  document.querySelector('.board_list');
 			...
 			if(selectNo == 0 || selectNo == 2) {
				 ...
				 for(let i = 0; i < result.length; i++) {
					html += `
						<tr>
			                <td class="no"> ${result[i].bno}  </td>
			                <td class="name"> ${result[i].id} </td>
			                <th onclick="boardDetailsChange(${result[i].bno})" class="title"> ${result[i].btitle} </a> </th>
			            	<td class="hits"> ${result[i].hits} </td> 
			           		<td class="time"> ${result[i].bdate} </td>                
		               	</tr>`;
				 }
		        ...
		        for(let i = 0; i < result.length; i++) {
					...
					html +=	` <tr>
		                  <td class="no"> ${result[i].bno} </td>
		                  <td class="name"> ${result[i].id} </td>
		                  <th class="menu"> ${result[i].pname} </th>
		                  <td onclick="boardDetailsChange(${result[i].bno})" class="title">${result[i].btitle} </td>
		                  <td class="star"> ${sum} ( ${num} / 5) </td> 
		                  <td class="hits"> ${result[i].hits} </td> 
		                  <td class="time">  ${result[i].bdate} </td>                
		               </tr>
		               `
