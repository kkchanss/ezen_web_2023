...
public class ServerSocket {
	...
	public void onClose(Session session) {
		...
		for(ClientDto clientDto : clientList) { //  접속목록에서 연결이 끊긴 세션 찾기
			if(clientDto.getSession() == session) {
				// 클라이언트소켓이 세션과 연결이 끊긴 세션과 같으면 해당 dto를 제거하기. 
	...
	// 2. 
	@OnMessage // 매개변수 : 1. 메시지를 보낸 클라이언트소켓(세션) 2. 메세지 내용 [문자열]
...
/*
	* WebSocket 서버 소켓 라이브러리 제공
		...
		2. 서버소켓 메소드 제공 
				...
				4. 클라이언트 소켓으로부터 메세지를 받았을때, 실행되는 메소드
				@OnMessage
					public void onClose(Session session, String msg) {}
						- Session : 서버소켓에게 메세지를 보낸 클라이언트소켓 정보
						- String 매개변수 : 클라이언트소켓으로부터 받은 메세지 
