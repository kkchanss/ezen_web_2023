/*
	...
	JS : 객체 지향 언어 [ OOP : Object Oriented Programming]
		...
		대량화/자동화 ==> 객체지향 프로그래밍 적합 하다.
			=> 미리 설계(클래스)하고 컴퓨터 설계 주입해서 설계기반으로 
		ex)
			- 현대 소나타
				공장 생성 => 소나타 생성 설계 기준
				A소나타=객체, B소나타=객체
			...
			- 23년신형 LG냉장고
				냉장고설계도 = 클래스
				냉장고 = 객체
			- 객체 안에 다른 객체 저장 가능 : 우리집(객체[냉장고(객체)])
				...
				- 배열 단점 : 서로 다른 정보 유형들을 하나의 배열에 관리 어렵다
					ex) 유재석/강호동/신동엽 3명의 회원( 정보 : 아이디, 비밀번호, 이름, 주소 등 ) 등록
						...
						3방법 : 객체유형
							...
							let 회원3 = { 아이디 : 'zxc', 비밀번호 : 'zxc', 이름 : '신동협', 주소 : '수원'}
			4. 객체 = { }
 			...
 			5. 객체 호출
 				1. 객체 전체 : 객체명
 				2. 객체 내 특정 속성 호출 : 
 					. 접근 연산자[ 객체 내 속성 접근(호출)헤사 데이터 호출]
 			6. 객체 속성 값 변경
 				객체명.속성명 = 새로운 값
 			...
 			9. 여러개 객체 => 관리 (배열 / 리스트 이용)
 				회원객체1, 1번 게시물 객체, 회원 객체2, 2번 게시물 객체
 ...
 */
 ...
 // 2. 객체 호출
 console.log(객체1) // 객체1
 console.log('아이디 속성 호출 : ' + 객체1.아이디) // 객체1의 특정(아이디) 속성 내 데이터 호출
 console.log('비밀번호 속성 호출 : ' + 객체1.비밀번호) // 객체 내 속성 알고 있으면 데이터  호출 가능하다.
 ...
 // 4. 객체 내 속성 추가
 객체1.주소 = '안산'; // [추가] 객체 내 주소 속성이 없으므로 호출이 아닌 추가돼서 값 대입
 console.log(객체1)
 ...
 let 객체2 = { 아이디 : 'asd', 비밀번호 : 'asd', 이름 : '강호동'}
 let 객체3 = { 아이디 : 'zxc', 비밀번호 : 'zxc', 이름 : '신동엽'}
 
 let 회원객체배열 = [객체1, 객체2, 객체3]
 console.log(회원객체배열)
 
 // !!!!!!!!!! : JSON ------> 서로 (웹/앱 관련) 언어들간의 데이터 구조 형태
